TVector3 GetDetPosition(double Angle, double Z, double D, double HalfWidth)
{
    ...
    double pos_Z = Z ;//+ fabs(HalfWidth*TMath::Sin(a));
    double pos_Y = 0;
...
void run_sim_COCOTIER()
{
    ...
    TString outFile = "roots/sim_12Cp2p_air.root";
    TString parFile = "roots/par.root";

    Bool_t magnet = kTRUE;
    //Double_t fieldScale = -1672.0/3584.; //   
    //Double_t fieldScale = -0.90; //max field//16C
    Double_t fieldScale = -0.75; //max field//12C
    //Double_t fieldScale = -0.75; //max field//Olivier
    //Double_t fieldScale = -0.98; //max field//132Sn
    //Double_t fieldScale = -0.9; //max field//132Sn
...
//    Int_t nEvents = 4264;
    ...
    double angle_RPC = 20.;//GLAD rotation angle
    //double angle = 14.;//GLAD rotation angle
    ...
    // Fi10 detector
//540 cm corresponds to GLAD exit
    //TVector3 fi10pos = GetDetPosition(angle, 540, 0., 0.);
/*    TVector3 fi10pos = GetDetPosition(angle, 600, 0., 0.);
    TVector3 fi10pos2 = GetDetPosition(angle, 700, -10, 0.);
    run->AddModule(new R3BFi10("fi10_s454.geo.root", {fi10pos.X(), fi10pos.Y(),fi10pos.Z()}, {"" ,90.0,90.0-angle,0.0}));
    run->AddModule(new R3BFi10("fidouble_s522.geo.root", {fi10pos2.X(), fi10pos2.Y(),fi10pos2.Z()}, {"" ,-90.0,angle,90.0}));
*/
    TVector3 fi10pos = GetDetPosition(angle, 560, 0., 0.);
    run->AddModule(new R3BFi11("double_big_fi11_air.geo.root", {fi10pos.X(), fi10pos.Y(),fi10pos.Z()}, {"" ,90.0,90-angle,0.0}));

    //TOFD for fragments
    //TVector3 tofd_pos_frag = GetDetPosition(angle, 1000, 0., 0.);
    //run->AddModule(new R3BTofd("tofd_s454.geo.root", {tofd_pos_frag.X(), tofd_pos_frag.Y(), tofd_pos_frag.Z()}, {"" ,-90.0, angle, 90.0}));
//    TVector3 tofd_pos_frag2 = GetDetPosition(angle, 1100, 0., 0.);
//    run->AddModule(new R3BTofd("tof_test.geo.root", {tofd_pos_frag2.X(), tofd_pos_frag2.Y(), tofd_pos_frag2.Z()}, {"" ,-90.0, angle, 90.0}));
    ...
    //TOFD for protons
    //TVector3 tofd_pos_prot = GetDetPosition(angle_RPC, 580, -70., 0.);
    //run->AddModule(new R3BTofd("tof_rpc.geo.root", {tofd_pos_prot.X(), tofd_pos_prot.Y(), tofd_pos_prot.Z()}, {"" ,-90.0, angle_RPC+3, 90.0}));
    ...
    */
    if (generator.CompareTo("ascii") == 0)
    {
        ...
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/132Sn_MDF.txt");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/cocktail_12C_400ev_protons_neutrons.txt");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/cocktail_16C_400ev.txt");
        R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/12C_4x100ev.txt");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/gen_valerii_src_prel_p_1p25.txt");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("/Users/acorsi/codes/code_SRC/quasi_ascii_12Cppn_1.25GeV.out");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("/Users/acorsi/codes/p2p/quasi_12C_1.25GeV.out");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/olivier.txt");
        //R3BAsciiGenerator* gen = new R3BAsciiGenerator("evgenerator/evgen_ASCII_protons_SRC.cpp");

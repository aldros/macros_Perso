...
void create_target_geo(const char* geoTag = "LiH")
{
  ...
  if (0 == strncmp(geoTag, "LiH", 3))        ConstructGeometry(pMedVac,pMed1,pMed2,pMed3,pMed4,pMed5,pMedSi,pMedGold);
...
void ConstructGeometry(TGeoMedium *pMedVac, TGeoMedium *pMed1,
  ...
  //Remove where FOOT are located
  TGeoShape* pFOOTTube = new TGeoTube("FOOTTube", 0., 15., 10);
  TGeoVolume* pFOOTTubeVol = new TGeoVolume("FOOTTubeTest", pFOOTTube, pMed2);
  pFOOTTubeVol->SetVisLeaves(kTRUE);
  pFOOTTubeVol->SetLineColor(34);
  TGeoCombiTrans* foot_tube = new TGeoCombiTrans("foot_tube", 0., 0., 12., fRefRot);
  foot_tube->RegisterYourself();
    
  //TGeoCompositeShape* pVCWorld = new TGeoCompositeShape("VCbox", "CenterPart:t_tube + Cone:t_cone");
  TGeoCompositeShape* pVCWorld = new TGeoCompositeShape("VCbox", "CenterPart:t_tube + Cone:t_cone - FOOTTube:foot_tube");
  ...
  //Test
  pTargetEnveloppe_log->AddNode(pFOOTTubeVol, 0, foot_tube);

			...
			TGeoMedium *pMed3, TGeoMedium *pMedAu)
{
  ...
  //dz   = 3.0250;
  Double_t rmax = 1*0.0050+2.11800;  
  dz   = 1*0.0050+2.518;
  TGeoShape *pTargetEnveloppe = new TGeoTube("TargetEnveloppe",rmin,rmax,dz);
  TGeoVolume *pTargetEnveloppe_log = new TGeoVolume("TargetEnveloppe",pTargetEnveloppe, pMed2); //Air
  pTargetEnveloppe_log->SetVisLeaves(kTRUE);
  TGeoCombiTrans* pGlobal = GetGlobalPosition(pMatrix2);
  //  gTop->AddNode(pTargetEnveloppe_log, 0, pGlobal);
  pWorld->AddNode(pTargetEnveloppe_log, 0, pGlobal);
  ...
  // Shape: Target1 type: TGeoTubeSeg - Target Cell
  // Mylar Tube
  rmin = 2.1;          //1.000000;
  //rmax = 3.5250;     //for 250 Mylar
  //rmax = 2.110;        //for 100 Mylar
  rmax = 2.118;        //for 180 Mylar
  dz   = 2.5;           //1.750000;
  ...
  // Shape: Target3 type: TGeoTubeSeg - Entrance/Exit windows
  rmin = 0.000000;
  //rmax = 3.5250;    //for 250 mylar;
  //dz   = 0.012500;  // =half thickness for Mylar
  //rmax = 2.110;     //for 100 mylar;
  rmax = 2.118;     //for 180um mylar;
  dz   = 0.00900; // =half thickness for Mylar
...
//pWorld->AddNode(pTarget4_log, 0, pMatrix12);
  ...
  // Shape: Target5 type: TGeoTubeSeg - Insulation of target cell (Cylinder)
  // Mylar Tube
  rmin = 2.118;          //1.000000;
  //rmax = 3.5250;     //for 250 Mylar
  rmax = 1*0.0050 + 2.118;        //for 10*5um Mylar
  dz   = 2.5180;           //1.750000;
  TGeoShape *pTarget5 = new TGeoTube("Target5",rmin,rmax,dz);
  TGeoVolume *pTarget5_log = new TGeoVolume("Target5",pTarget5, pMed15);   // Mylar
  pTarget5_log->SetVisLeaves(kTRUE);
  pTargetEnveloppe_log->AddNode(pTarget5_log, 0, pMatrix4);
  
  // Shape: Target6 type: TGeoTubeSeg - Insulation of target cell (Exit window)
  rmin = 0.000000;
  //rmax = 3.5250;    //for 250 mylar;
  //dz   = 0.012500;  // =half thickness for Mylar
  rmax = 1*0.0050+2.118;     //for 10*5um mylar;
  dz   = 1*0.00250; // =half thickness for Mylar (10*5um)
  TGeoShape *pTarget6 = new TGeoTube("Target6",rmin,rmax,dz);
  TGeoVolume *pTarget6_log = new TGeoVolume("Target6",pTarget6, pMed15);    // Mylar
  pTarget6_log->SetVisLeaves(kTRUE);
  pTargetEnveloppe_log->AddNode(pTarget6_log, 0, pMatrix14);
  // pTargetEnveloppe_log->AddNode(pTarget6_log, 1, pMatrix10);
}
